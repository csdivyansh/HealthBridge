"""
HealthBridge v2 — Production-Grade Training Pipeline
=====================================================
Why a Pipeline?
...
def build_pipeline(n_classes: int) -> Pipeline:
    """Return an unfitted Pipeline: FeatureEngineer → ColumnTransformer → XGB."""

    # --- Numerical sub-pipeline: impute → scale --------------------------
    numerical_pipeline = Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("scaler",  RobustScaler()),
    ])

    # --- Categorical sub-pipeline: impute → one-hot ---------------------
...
def train(
    ...
    """End-to-end training: load → split → tune → evaluate → save."""
    ...
    if tune:
        logger.info("Starting RandomizedSearchCV (5-fold Stratified) …")
        ...
        logger.info("Training pipeline (no tuning) …")
    ...
    pipeline_path = os.path.join(output_dir, "healthbridge_pipeline_v2.joblib")
    ...
    try:
        ...
        plt.title("Confusion Matrix — HealthBridge v2")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="HealthBridge v2 — Train ML Pipeline")
