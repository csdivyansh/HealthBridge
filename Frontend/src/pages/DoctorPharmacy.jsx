        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Pharmacy Network</h1>
                    ...
                    {[
                        ...
                        <button key={t.key} onClick={() => { setTab(t.key); setSelectedPharmacy(null); }}
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all ${
                                tab === t.key ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'
                            }`}>
                    ...
                    <div>
                        <div className="relative mb-6">
                            ...
                            <input value={search} onChange={e => setSearch(e.target.value)}
                                ...
                                className="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
                        </div>
                            ...
                            {filteredPharmacies.map(p => (
                                <div key={p._id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-lg transition-all group">
                                    ...
                                    <button onClick={() => handleViewPharmacy(p)}
                                        className="w-full py-2.5 bg-gradient-to-r from-indigo-50 to-violet-50 text-indigo-600 font-medium rounded-xl text-sm hover:from-indigo-100 hover:to-violet-100 transition flex items-center justify-center gap-2 group-hover:from-indigo-600 group-hover:to-violet-600 group-hover:text-white">
                    ...
                    <div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <div className="p-6">
                                ...
                                {loadingMeds ? (
                                    <div className="text-center py-8"><Loader2 className="w-8 h-8 text-indigo-500 animate-spin mx-auto" /></div>
                                ...
                                {prescriptions.filter(rx => rx.status === 'created').length > 0 && (
                                    <div className="mt-6 pt-6 border-t border-gray-200">
                                        <h4 className="font-semibold text-gray-700 mb-3 flex items-center gap-2"><Send className="w-4 h-4 text-indigo-500" /> Send Prescription Here</h4>
                                        {prescriptions.filter(rx => rx.status === 'created').map(rx => (
                                            <div key={rx._id} className="p-3 bg-indigo-50 rounded-xl flex items-center justify-between mb-2">
                                                <div>
                    ...
                    <div>
                        {prescriptions.length === 0 ? (
                            <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
                                ...
                                <button onClick={() => setTab('new')} className="mt-4 px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-medium text-sm hover:bg-indigo-700 transition">
                ...
                {tab === 'new' && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 p-6 text-white">
                            ...
                            <p className="text-indigo-200 text-sm mt-1">Fill in the details and send to a pharmacy</p>
                        </div>
                        <form onSubmit={handleCreatePrescription} className="p-6 space-y-6">
                                    ...
                                    {myPatients.length > 0 ? (
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full border border-gray-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent mb-2" />
                                            <div className="max-h-36 overflow-y-auto border border-gray-200 rounded-xl divide-y divide-gray-100">
                                                {myPatients
                                                    ...
                                                    <button type="button" key={p._id} onClick={() => { setRxForm(prev => ({ ...prev, patientId: p._id })); setPatientSearch(p.name); }}
                                                        className={`w-full text-left px-4 py-2.5 text-sm hover:bg-indigo-50 transition flex items-center justify-between ${rxForm.patientId === p._id ? 'bg-indigo-50 border-l-2 border-indigo-500' : ''}`}>
                                                        <div>
                                                            <p className="font-semibold text-gray-800">{p.name}</p>
                                                            <p className="text-xs text-gray-500">{p.email} {p.age ? `· Age: ${p.age}` : ''} {p.gender ? `· ${p.gender}` : ''}</p>
                                                        </div>
                                                        {rxForm.patientId === p._id && <CheckCircle className="w-4 h-4 text-indigo-600" />}
                                                    </button>
                                                ))}
                                        ...
                                        <div className="p-4 bg-amber-50 rounded-xl border border-amber-200">
                                            ...
                                            <input value={rxForm.patientId} onChange={e => setRxForm(prev => ({ ...prev, patientId: e.target.value }))}
                                                placeholder="Or enter patient ID manually" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-2 focus:ring-2 focus:ring-indigo-500" />
                                        </div>
                                    )}
                                </div>
                                <div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <button type="button" onClick={addMedicineLine}
                                        className="text-indigo-600 text-sm font-medium flex items-center gap-1 hover:text-indigo-800 transition">
                            ...
                            <button type="submit" disabled={submittingRx}
                                className="w-full py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-bold rounded-xl hover:from-indigo-700 hover:to-violet-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2 shadow-lg">
