...
const statusConfig = {
    ...
    received: { label: 'Received by Pharmacy', color: 'bg-indigo-100 text-indigo-700', icon: Package },
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">
            <div className="max-w-5xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">My Prescriptions</h1>
                ...
                <div className="flex gap-2 mb-6 flex-wrap">
                    {[
                        ...
                        <button key={t.key} onClick={() => setFilter(t.key)}
                            className={`px-4 py-2 rounded-xl text-sm font-medium transition ${filter === t.key ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-gray-600 hover:bg-gray-50 border'}`}>
                            {t.label}
                        </button>
                    ))}
                </div>
                                ...
                                <div key={rx._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all">
                                    ...
                                    <button onClick={() => setSelectedPrescription(isExpanded ? null : rx._id)}
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-violet-600 rounded-xl flex items-center justify-center shadow">
                                                <FileText className="w-6 h-6 text-white" />
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-gray-800">{rx.diagnosis}</h3>
                                                <p className="text-sm text-gray-500">Dr. {rx.doctorId?.name} • {rx.doctorId?.specialization}</p>
                                                <p className="text-xs text-gray-400 mt-1">{new Date(rx.createdAt).toLocaleDateString()} • {rx.medicines?.length} medication(s)</p>
                                            </div>
                                        </div>
                                    ...
                                    {isExpanded && (
                                        <div className="px-5 pb-5 border-t border-gray-100">
                                            ...
                                            {/* Medicines */}
                                            <h4 className="font-semibold text-gray-700 text-sm mb-3 flex items-center gap-2"><Pill className="w-4 h-4 text-indigo-500" /> Medications</h4>
                                                ...
                                                {rx.medicines?.map((med, idx) => (
                                                    <div key={idx} className="p-3 bg-gray-50 rounded-xl flex items-center justify-between">
                                                        <div>
                                                            ...
                                                            {med.instructions && <p className="text-xs text-indigo-500 mt-1">{med.instructions}</p>}
                                                        </div>
                                                        <span className="text-xs bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-medium">Qty: {med.quantity}</span>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            {rx.pharmacyId && (
                                                <div className="p-3 bg-green-50 rounded-xl mb-4 flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-500">{rx.pharmacyId.address} • {rx.pharmacyId.phone}</p>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {rx.status === 'created' && (
                                                <button onClick={() => handleCheckAvailability(rx._id)}
                                                    ...
                                                    className="w-full py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-medium rounded-xl text-sm hover:from-indigo-700 hover:to-violet-700 transition-all flex items-center justify-center gap-2">
