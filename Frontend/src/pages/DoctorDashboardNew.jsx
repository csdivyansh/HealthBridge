...
const DoctorDashboardNew = () => {
    ...
    return (
        <div className={`h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50/30 flex flex-col overflow-hidden font-sans text-gray-900 ${isOffline ? 'grayscale-[30%]' : ''}`}>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative hidden md:block">
                        ...
                        <input 
                            ...
                            className="pl-9 pr-4 py-2 bg-gray-100 border-0 rounded-lg text-sm w-48 focus:ring-2 focus:ring-indigo-500 outline-none"
                        />
                        {searchQuery && (
                    ...
                    <button 
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 text-purple-600 bg-purple-50 hover:bg-purple-100 rounded-lg transition text-xs font-semibold"
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 flex flex-col max-h-[80vh]">
                        <div className="flex justify-between items-center mb-4 shrink-0">
                            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                <Brain className="text-purple-600 w-5 h-5"/> Patient Case Summary
                            </h2>
                            <button onClick={() => setShowAIModal(false)}>
                                <X className="w-5 h-5 text-gray-500"/>
                            </button>
                        </div>
                        <div className="flex-1 overflow-y-auto bg-gradient-to-br from-purple-50 to-indigo-50 p-4 rounded-xl border border-purple-100 text-sm leading-relaxed text-gray-700">
                            {aiLoading ? (
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
                        ...
                        <form onSubmit={handleReschedule} className="space-y-4">
                            ...
                            <div className="flex gap-2 justify-end mt-4">
                                ...
                                <button 
                                    type="submit" 
                                    className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-bold shadow-lg shadow-indigo-200"
                                >
                                    Confirm
                                </button>
                            </div>
            ...
            {showAIToolsPanel && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in zoom-in duration-200">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col overflow-hidden">
                        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 py-4 flex justify-between items-center shrink-0">
                            <h3 className="font-bold flex items-center gap-2">
                                <Sparkles className="w-5 h-5" /> AI-Powered Doctor Tools
                            </h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            {activeAITool === 'menu' && (
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <button onClick={() => setActiveAITool('clinical-notes-form')} className="p-5 bg-purple-50 hover:bg-purple-100 rounded-2xl border border-purple-200 text-left transition group">
                                        <FileText className="w-8 h-8 text-purple-600 mb-2 group-hover:scale-110 transition" />
                            ...
                            {activeAITool === 'treatment-plan' && !aiToolResult && (
                                <div className="space-y-4">
                                    <button onClick={() => setActiveAITool('menu')} className="text-sm text-purple-600 font-bold hover:underline">&larr; Back to tools</button>
                                    ...
                                    <div className="space-y-3">
                                        <input type="text" placeholder="Diagnosis *" value={treatmentPlanForm.diagnosis} onChange={e => setTreatmentPlanForm({...treatmentPlanForm, diagnosis: e.target.value})} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-200" />
                                        <div className="grid grid-cols-2 gap-3">
                                            <input type="number" placeholder="Patient Age" value={treatmentPlanForm.patientAge} onChange={e => setTreatmentPlanForm({...treatmentPlanForm, patientAge: e.target.value})} className="p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-200" />
                                        ...
                                        <textarea placeholder="Current Symptoms" value={treatmentPlanForm.symptoms} onChange={e => setTreatmentPlanForm({...treatmentPlanForm, symptoms: e.target.value})} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-200 h-24 resize-none" />
                                        <button onClick={handleGenerateTreatmentPlan} disabled={aiToolLoading || !treatmentPlanForm.diagnosis} className="w-full py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2">
                            ...
                            {activeAITool === 'certificate' && !aiToolResult && (
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        <div className="grid grid-cols-2 gap-3">
                                            <input type="text" placeholder="Patient Name *" value={certificateForm.patientName} onChange={e => setCertificateForm({...certificateForm, patientName: e.target.value})} className="p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-200" />
                                            <input type="number" placeholder="Age" value={certificateForm.patientAge} onChange={e => setCertificateForm({...certificateForm, patientAge: e.target.value})} className="p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-200" />
                                        </div>
                                        <input type="text" placeholder="Diagnosis / Condition *" value={certificateForm.diagnosis} onChange={e => setCertificateForm({...certificateForm, diagnosis: e.target.value})} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-200" />
                                        <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 max-h-60 overflow-y-auto">
                                        ...
                                        {appointments.filter(a => a.status === 'completed').map(apt => (
                                            <button key={apt._id} onClick={() => handleGenerateClinicalNotes(apt._id)} className="w-full p-3 bg-gray-50 hover:bg-purple-50 rounded-xl border text-left transition flex items-center justify-between">
                                                <div>
                            ...
                            {aiToolResult && (
                                <div className="space-y-4">
                                    <button onClick={() => { setActiveAITool('menu'); setAiToolResult(''); }} className="text-sm text-purple-600 font-bold hover:underline">&larr; Back to tools</button>
                                    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 p-5 rounded-xl border border-purple-100 text-sm text-gray-700 whitespace-pre-wrap leading-relaxed max-h-[50vh] overflow-y-auto">
                                        {aiToolResult}
                                    </div>
                                    <div className="flex gap-3">
                                        ...
                                        <button onClick={() => { setActiveAITool('menu'); setAiToolResult(''); }} className="flex-1 py-2.5 bg-purple-600 text-white rounded-xl font-bold text-sm hover:bg-purple-700">Done</button>
                                    </div>
                                </div>
                            )}
