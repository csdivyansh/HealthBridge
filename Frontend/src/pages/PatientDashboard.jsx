...
const DashboardCard = ({ title, Icon, colorKey, description, onClick }) => {
  const colors = {
    indigo: { bg: "bg-indigo-50", icon: "text-indigo-600", hover: "group-hover:bg-indigo-100", shadow: "group-hover:shadow-indigo-500/10" },
    ...
    purple: { bg: "bg-violet-50", icon: "text-violet-600", hover: "group-hover:bg-violet-100", shadow: "group-hover:shadow-violet-500/10" },
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-end flex-wrap">
          {isCompleted ? (
            ...
            <>
              <button onClick={() => onChat(apt)} className="p-2.5 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition" title="Chat"><MessageSquare className="h-4 w-4" /></button>
            ...
            <>
              <button onClick={() => onChat(apt)} className="p-2.5 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition" title="Chat with Doctor"><MessageSquare className="h-4 w-4" /></button>
...
const PatientDashboard = () => {
  ...
  const handleDownloadReceipt = (apt) => {
    const doc = new jsPDF();
    doc.setFontSize(20);
    doc.text("HealthBridge - Consultation Receipt", 20, 20);
    doc.setFontSize(12);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50/30 p-4 md:p-8 font-sans text-gray-900 relative">
      
      {/* FLOATING AI BUTTON */}
      <button onClick={() => navigate("/patient/ai-assistant")} className="fixed bottom-8 right-8 z-40 bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 text-white p-4 rounded-2xl shadow-2xl shadow-indigo-500/30 hover:scale-110 hover:shadow-indigo-500/40 transition-all duration-300 group">
      ...
      {notification && (
        <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-4 ${
          notification.type === 'reminder' ? 'bg-gradient-to-r from-indigo-600 to-violet-600' : 'bg-gradient-to-r from-emerald-500 to-teal-500'
        } text-white`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleRescheduleSubmit} className="space-y-4">
              <div>
                ...
                <input type="date" required value={rescheduleData.date} onChange={e => setRescheduleData({...rescheduleData, date: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Preferred Time</label>
                <select required value={rescheduleData.time} onChange={e => setRescheduleData({...rescheduleData, time: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700">Request Reschedule</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea value={reviewData.comment} onChange={e => setReviewData({...reviewData, comment: e.target.value})} rows={3} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none resize-none" placeholder="Share your experience..." />
              </div>
              <div className="flex gap-3 pt-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 p-4 bg-indigo-50 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-indigo-600 text-sm">{selectedAppointment.doctorId?.specialization}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-gray-50 rounded-xl">
                  ...
                  <p className={`font-bold ${selectedAppointment.status === 'completed' ? 'text-green-600' : 'text-indigo-600'}`}>{selectedAppointment.status}</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-xl">
              ...
              {selectedAppointment.prescription && (
                <div className="p-4 bg-violet-50 rounded-xl">
                  <p className="text-xs font-bold text-violet-600 uppercase mb-2">Prescription</p>
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button onClick={() => setShowDetailsModal(false)} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* TOP HEADER */}
      <header className="flex flex-col md:flex-row justify-between items-end bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 text-white p-8 rounded-3xl shadow-xl shadow-indigo-500/20 relative overflow-hidden mb-8">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[size:32px_32px]"></div>
        <div className="absolute top-0 right-0 w-96 h-96 bg-white/5 rounded-full blur-3xl"></div>
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-white/5 rounded-full blur-3xl"></div>
        <div className="relative z-10">
            <p className="text-indigo-200 font-medium mb-1 text-sm">Welcome back,</p>
        ...
        <div className="flex bg-white/15 backdrop-blur-md p-1.5 rounded-2xl mt-6 md:mt-0 relative z-10 border border-white/20">
            {['overview', 'health', 'appointments'].map(tab => (
                <button 
                    ...
                    className={`px-6 py-2.5 rounded-xl text-sm font-bold capitalize transition-all duration-300 ${activeTab === tab ? 'bg-white text-indigo-600 shadow-lg' : 'text-white/80 hover:text-white hover:bg-white/10'}`}
                >
                    {tab}
                </button>
            ))}
        ...
        {activeTab === 'overview' && (
            <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex-1 bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-6 rounded-2xl shadow-xl shadow-indigo-500/20 relative overflow-hidden flex items-center justify-between">
                        ...
                        <div className="relative z-10">
                            ...
                            {nextAppt ? (
                                <div>
                                    ...
                                    <p className="text-indigo-100 flex items-center gap-2 mt-2"><Clock className="w-4 h-4"/> {new Date(nextAppt.date).toLocaleDateString()} at {nextAppt.timeSlot}</p>
                                </div>
                            ) : (
                                <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 flex-1">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-gray-200 transition-all duration-300">
                            ...
                            <p className="text-3xl font-black bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent mt-1">{appointments.length}</p>
                        </div>
                    </div>
                </div>

                {/* 7 CARDS GRID (Included New Feature) */}
                <h2 className="text-xl font-bold text-gray-900 flex items-center gap-3"><LayoutGrid className="w-6 h-6 text-indigo-600"/> Quick Access</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                    ...
                    <DashboardCard title="My Prescriptions" Icon={ClipboardList} colorKey="indigo" description="Download prescriptions as PDF." onClick={() => navigate("/patient/prescriptions")} />
                    ...
                    <DashboardCard title="Wellness Score" Icon={HeartPulse} colorKey="purple" description="AI-computed health score." onClick={() => navigate("/patient/wellness-score")} />
                    ...
                    <DashboardCard title="Health Analytics" Icon={BarChart3} colorKey="sky" description="Track health trends & progress." onClick={() => navigate("/patient/health-analytics")} />
