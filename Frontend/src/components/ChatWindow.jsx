...
const ChatWindow = ({ partner, onEndChat, userRole }) => {
    ...
    return (
        ...
        <div className="flex flex-col h-full min-h-0 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden relative">
            ...
            {/* HEADER */}
            <div className="px-6 py-4 bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 flex justify-between items-center shrink-0 relative overflow-hidden">
                ...
                {messages.map((msg, index) => {
                    ...
                    return (
                        <div key={msg._id || index} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 fade-in duration-300`}>
                            <div className={`max-w-[75%] p-4 rounded-2xl shadow-sm relative text-sm ${isMe ? 'bg-gradient-to-br from-indigo-600 to-violet-600 text-white rounded-br-sm' : 'bg-white text-gray-800 border border-gray-100 rounded-bl-sm shadow-md'}`}>
            ...
            <form onSubmit={sendMessage} className="p-4 bg-white border-t border-gray-100 flex items-center gap-3 shrink-0">
                ...
                <button type="button" onClick={() => fileInputRef.current.click()} disabled={isUploading || !isConnected} className="p-3 bg-gray-100 text-gray-500 rounded-xl hover:bg-gray-200 transition-all" title="Attach File">
                    {isUploading ? <div className="w-5 h-5 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div> : <Paperclip className="w-5 h-5" />}
                </button>
                ...
                <input 
                    ...
                    className="flex-1 px-4 py-3 bg-gray-100 border-2 border-transparent focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 rounded-xl transition-all outline-none text-sm font-medium placeholder-gray-400"
                    disabled={!isConnected || isUploading} 
                />
                
                <button type="submit" disabled={!isConnected || (input.trim() === '' && !isUploading)} className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-3 rounded-xl hover:shadow-lg hover:shadow-indigo-500/30 disabled:opacity-50 disabled:cursor-not-allowed transition-all transform active:scale-95">
